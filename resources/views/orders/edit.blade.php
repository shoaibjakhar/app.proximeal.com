...
@push('css_lib')
    ...
    <link rel="stylesheet" href="{{ asset('dist/css/ohsnap.css') }}">
        ...
        <div class="card">
            <div class="card-header">
                <ul class="nav nav-tabs align-items-end card-header-tabs w-100">
                    ...
                    @can('orders.create')
                        <li class="nav-item">
                            <a class="nav-link" href="{!! route('orders.create') !!}"><i
                                    class="fa fa-plus mr-2"></i>{{ trans('lang.order_create') }}</a>
                        </li>
                    @endcan
                    <li class="nav-item">
                        <a class="nav-link active" href="{!! url()->current() !!}"><i
                                class="fa fa-pencil mr-2"></i>{{ trans('lang.order_edit') }}</a>
                    </li>
                </ul>
            </div>
            <div class="card-body">
...
@push('scripts_lib')
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#btn-assign-order').click(function(e) {
                console.log("CLICKED ASSIGN ORDER");

                if (order_status_id == null || order_status_id != 2) {
                    alert("Order is not preparing phase");
                    return;
                }

                alert("The process of assigning order is started.");
                $.ajax({
                    type: "POST",
                    url: '/api/orders/assign/' + "{{ $order->id }}/" + order_status_id,
                    data: {
                        _token: '{{ csrf_token() }}',
                    },
