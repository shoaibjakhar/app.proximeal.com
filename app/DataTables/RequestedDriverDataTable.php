...
class RequestedDriverDataTable extends DataTable
{
    /**
    ...
    protected function getColumns()
    {
        $columns = [
            ...
            [
                'data' => 'active',
                'title' => trans('lang.driver_active'),
    ...
    public function query(Driver $model)
    {
        ...
        if(auth()->user()->hasRole('admin')){
            return $model->newQuery()->with("user")->select('drivers.*')->where('drivers.active', '0')->orwhere('drivers.status', '!=', 'Approved');;
