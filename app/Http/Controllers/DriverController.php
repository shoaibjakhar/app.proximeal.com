...
use App\DataTables\RequestedDriverDataTable;
...
class DriverController extends Controller
{
  ...
  */
    ...
    public function requestedDrivers(RequestedDriverDataTable $RequestedDriverDataTable)
    {
       // die('here');
        return $RequestedDriverDataTable->render('drivers.requested');
      //  print_r($requestedRestaurantDataTable->render('restaurants.requested'));
    }
    ...
    public function edit($id)
    {
        ...
        $user_id_new=DB::table('drivers')->where('id', $id)->value('user_id');
        $driver_user=DB::table('users')->where('id', $user_id_new)->get()->toArray();
        ...
        return view('drivers.edit')->with('driver', $driver)->with("customFields", isset($html) ? $html : false)->with("user",$user)->with('driver_data',$data)->with('driver_user',$driver_user);
    }
    ...
    public function update($id, UpdateDriverRequest $request)
    {
        ...
        $D_data['delivery_fee']=$input['delivery_fee'];
        $D_data['status']=$input['status'];
        $D_data['active']=$input['active'];

        $userData['name']=$input['name'];
        $userData['email']=$input['email'];
        $userData['phone']=$input['phone'];
        $userData['info']=$input['info'];

        $user_id_new=DB::table('drivers')->where('id', $id)->value('user_id');
       
          $user = $this->userRepository->update($userData, $user_id_new);
        
        $customFields = $this->customFieldRepository->findByField('custom_field_model', $this->driverRepository->model());
        try {
            $driver = $this->driverRepository->update($D_data, $id);
