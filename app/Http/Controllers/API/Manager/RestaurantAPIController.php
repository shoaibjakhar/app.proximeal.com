...
class RestaurantAPIController extends Controller
{
    ...
    public function getFoods(){
        ...
        foreach($restaurants as $rest){
            $rsFoods = Food::where('restaurant_id', $rest->id)->get();
            if(!empty($rsFoods)){
                foreach($rsFoods as $rsfood){
                    array_push($foods, $rsfood);
                }
            }
        }
        return $foods;
    ...
    public function getExtras()
    {
        ...
        foreach ($extras as $extra) {
            $extra['food'] = Food::find($extra->food_id);
            $extra['extra_group'] = ExtraGroup::find($extra->extra_group_id);
        }

        return $this->sendResponse($extras, 'Extras retrieved successfully.');
    }
}
